...
(function () {
  ...
  window.route = {
    ...
    caffetteria: function (ctx, next) {
      get('../views/caffetteria.html', function (html) {
        ctx.data.index = 1;
        ctx.partials.content = html;
        next();
      });
    },
    cocktail: function (ctx, next) {
      get('../views/cocktail.html', function (html) {
        ctx.data.index = 2;
        ctx.partials.content = html;
        next();
      });
    ...
    info: function (ctx, next) {
      get('../views/info.html', function (html) {
        ctx.data.index = 3;
        ctx.partials.content = html;
        next();
      });
